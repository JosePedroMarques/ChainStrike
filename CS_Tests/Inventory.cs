...
namespace CS_Tests
{
    ...
    public class Inventory
    {
       ...
       /* private Tuple<Node,Cost, List<Node>,Tuple<int, int, int, int, int>> GetUnit(int stars)
        ...
        public void Go()
        {
            var bestCostPerStar = new EvolutionOutcome[5];
        ...
        private EvolutionOutcome GetBestCostPerStar(int stars, EvolutionOutcome[] bestCostPerStar)
        {
            ...
            foreach (var possibleInput in possibleInputs)
            {
                ...
                var input = new Node[possibleInput.Level1 + possibleInput.Level2 + possibleInput.Level3 +
                                     possibleInput.Level4 + possibleInput.Level5];
                int count = 0;
                for (var i = 0; i < possibleInput.Level1; i++)
                {
                    var unit = GetUnit(1, current, bestCostPerStar);
                    input[count++] = unit;
                }
                for (var i = 0; i < possibleInput.Level2; i++)
                {
                    var unit = GetUnit(2, current, bestCostPerStar);
                    input[count++] = unit;
                }
                for (var i = 0; i < possibleInput.Level3; i++)
                {
                    var unit = GetUnit(3, current, bestCostPerStar);
                    input[count++] = unit;
                }
                for (var i = 0; i < possibleInput.Level4; i++)
                {
                    var unit = GetUnit(4, current, bestCostPerStar);
                    input[count++] = unit;
                }
                for (var i = 0; i < possibleInput.Level5; i++)
                {
                    var unit = GetUnit(5, current, bestCostPerStar);
                    input[count++] = unit;
                }
                var baseUnit = GetUnit(stars-1,current, bestCostPerStar);
                baseUnit.MakeMaxLevel(current.Cost);
                baseUnit.ApplyPowerUp(current.Cost, input);
        ...
        private Node GetUnit(int stars, EvolutionOutcome current, EvolutionOutcome[] bestCostPerStar)
        {
            ...
            if (current.Units.Bag[stars] > 0)
            {
                current.Units.Decrement(stars);
                return new Node(stars);
            }
            //else we need to make a new one
           current.Add(bestCostPerStar[stars-1]);
            return new Node(stars);
